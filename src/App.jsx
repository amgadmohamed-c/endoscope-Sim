...
export default function App() {
  ...
  const [logEntries, setLogEntries] = useState([
    { msg: 'System initialized',      type: 'info', time: '00:00', acked: false },
    { msg: 'Scenario: HEALTHY COLON', type: 'info', time: '00:00', acked: false },
  ])

  const notify = useNotifications()
  const buzz   = useAlarmSound()
  ...
  const addLog = useCallback((msg, type) => {
    ...
    setLogEntries(prev => [{ msg, type, time, acked: false }, ...prev].slice(0, 40))
  }, [])

  const handleAck = useCallback(() => {
    setLogEntries(prev =>
      prev.map(e => e.type === 'danger' && !e.acked ? { ...e, acked: true } : e)
    )
    setAlarming(false)
    addLog('Alarm acknowledged by operator', 'info')
  }, [addLog])
  ...
  const handleScenarioChange = (id) => {
    ...
    addLog(`Scenario loaded: ${id.toUpperCase().replace('_',' ')}`, 'info')
  }
