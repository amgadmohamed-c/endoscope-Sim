...
function buildColonGeometry(scenario) {
  ...
  // Haustra folds — thin crescent shelves (plicae semilunares)
  // Use TorusGeometry but with a very small tube radius so folds are flat/thin like real tissue.
  // Real folds protrude only ~3-5% of lumen radius — NOT 24% like before.
  ...
  const foldMat   = new THREE.MeshStandardMaterial({
    map: foldTex, normalMap: normalTex, normalScale: new THREE.Vector2(1.4, 1.4),
    roughnessMap: roughTex, roughness: 0.55, metalness: 0.02,
    emissive: new THREE.Color(...cfg.emissive), emissiveIntensity: 0.75,
  ...
  for (let f = 0; f < cfg.folds; f++) {
    ...
    // Full ring — haustral folds go all the way around the colon like a washer
    const foldR  = cfg.tubeR * 0.91
    // Thin cross-section so they look like flat tissue shelves, not sausages
    const thick  = cfg.tubeR * (0.030 + Math.sin(f * 1.7) * 0.006)

    const fold = new THREE.Mesh(
      new THREE.TorusGeometry(foldR, thick, 6, 28),
      foldMat
    )
    fold.quaternion.setFromRotationMatrix(
      new THREE.Matrix4().makeBasis(right, realUp, tan.clone().negate())
    )
